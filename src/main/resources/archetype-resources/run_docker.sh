...
# Java EE 7 servers
docker run --rm --name appsvr -p 80:8080 -v $(pwd)/target/${artifactId}.war:/opt/jboss/wildfly/standalone/deployments/${artifactId}.war jboss/wildfly:10.1.0.Final /opt/jboss/wildfly/bin/standalone.sh --server-config=standalone-full.xml -b "0.0.0.0"
